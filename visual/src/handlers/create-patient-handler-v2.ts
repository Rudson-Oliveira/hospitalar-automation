/**
 * Handler para Criação de Paciente no Sistema Hospitalar - V2 CORRIGIDO
 * Usa seletores corretos mapeados em 17/12/2025
 * Formulário simplificado: Nome + Cidade + Convênio
 */
 ...
 */
export async function createPatientSimple(
  page: Page,
  ...
  try {
    console.log('[CreatePatientV2] Iniciando criação de paciente:', patientData.nome);
    ...
    console.log('[CreatePatientV2] Página carregada');
    ...
    if (currentUrl.includes('login')) {
      ...
      console.log('[CreatePatientV2] Login necessário');
    ...
    console.log('[CreatePatientV2] Página de pacientes carregada');
    await new Promise(resolve => setTimeout(resolve, 2000));
    ...
    if (patientData.cidade) {
      ...
      if (await cidadeField.isVisible({ timeout: 5000 }).catch(() => false)) {
        ...
        steps.push(`✅ Cidade preenchida: ${patientData.cidade}`);
        await new Promise(resolve => setTimeout(resolve, 500));
        
        // Aguardar autocomplete e pressionar Enter
        await new Promise(resolve => setTimeout(resolve, 1000));
        await cidadeField.press('Enter');
        steps.push('✅ Enter pressionado no campo Cidade');
      } else {
        steps.push('⚠️  Campo Cidade (#inputCidade) não encontrado');
      }
    }
    
    // Passo 8: Preencher campo CONVÊNIO (obrigatório)
    ...
    if (await convenioField.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      // Aguardar autocomplete e pressionar Enter
      await new Promise(resolve => setTimeout(resolve, 1000));
      await convenioField.press('Enter');
      steps.push('✅ Enter pressionado no campo Convênio');
    } else {
    ...
    console.error('[CreatePatientV2] Erro:', error);
    ...
    try {
      ...
      console.error('[CreatePatientV2] Erro ao capturar screenshot:', e);
    }
